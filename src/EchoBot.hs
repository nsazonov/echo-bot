...
isCommand h c message = case hTextFromMessage h message of
  Nothing -> False
  Just messageText -> c == messageText

handleHelpCommand :: Monad m => Handle m a -> m [Response a]
handleHelpCommand h = do
  Logger.info (hLogHandle h) "Got the help command"
  let helpMessage = confHelpReply $ hConfig h
  return [MessageResponse $ hMessageFromText h helpMessage]

handleSettingRepetitionCount :: Monad m => Handle m a -> Int -> m [Response a]
handleSettingRepetitionCount h count = do
  Logger.info (hLogHandle h) $ "The user has set the repetition count to " .< count
  error "Not implemented"

handleRepeatCommand :: Monad m => Handle m a -> m [Response a]
handleRepeatCommand h = do
  Logger.info (hLogHandle h) "Got the repeat command"
  let template = confRepeatReply $ hConfig h
  let repeatCount = T.pack $ show $ confRepetitionCount $ hConfig h
  let reply = T.replace template "{count}" repeatCount
  return [MessageResponse (hMessageFromText h reply)]
...
respondWithEchoedMessage h message = do
  ...
  let repeatCount = confRepetitionCount $ hConfig h
  let response = MessageResponse  message
  return $ replicate repeatCount response
